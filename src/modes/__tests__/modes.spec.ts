...
test('should filter with custom mode tokens described in tokensToSkip',
     (t: ExecutionContext) => {
       const token = {type : 'comment'};
       t.false(getModeByName('custom')(token as IToken,
                                       {tokensToSkip : [ 'comment' ]}));
     });

test('should throw error with custom mode if tokensToSkip does not exists',
     (t: ExecutionContext) => {
       const token = {type : 'comment'};
       t.throws(() => { getModeByName('custom')(token as IToken); });
     });
