...
cli.option('-l, --min-lines [number]',
           'min size of duplication in code lines (Default is ' +
               getOption('minLines') + ')');
cli.option('-k, --min-tokens [number]',
           'min size of duplication in code tokens (Default is ' +
               getOption('minTokens') + ')');
cli.option('-x, --max-lines [number]',
           'max size of source in lines (Default is ' + getOption('maxLines') +
               ')');
cli.option(
    '-z, --max-size [string]',
    'max size of source in bytes, examples: 1kb, 1mb, 120kb (Default is ' +
        getOption('maxSize') + ')');
cli.option(
    '-t, --threshold [number]',
    'threshold for duplication, in case duplications >= threshold jscpd will exit with error');
cli.option('-c, --config [string]',
...
cli.option(
    '-m, --mode [string]',
    'mode of quality of search, can be "strict", "mild" and "weak" (Default is "' +
        getOption('mode') + '")');
cli.option(
    '-f, --format [string]',
...
cli.option('--formats-exts [string]',
...
cli.option('-p, --path [string]',
           '(Deprecated) Path to repo, use `jscpd <path>`');
...
clones.then(() => { StoresManager.close(); });
