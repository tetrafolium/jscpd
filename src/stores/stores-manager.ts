...
import {
  IStoreManagerOptions
} from '../interfaces/store/store-manager-options.interface';
...
export class StoreManager<T extends IStoreValue> {
  private registeredStores:
      {[name: string]: new(options: IStoreOptions) => IStore<T>;} =
          {memory : MemoryStore, files: FilesStore, leveldb: LevelDbStore};
  ...
  public flush() { this.stores = {}; }

  public close() {
    return Promise.all(Object.values(this.stores).map(store => store.close()))
        .then(() => {
          this.flush();
  ...
  public has(name: string): boolean { return this.stores.hasOwnProperty(name); }

  public getRegisteredStore(type: string):
  ...
  public create(name: string): void {
    if (!this.has(name)) {
      ...
      const {type, options = {}} = this.options[name] || this.options[main] ||
                                   this.options['*'] || {type : 'leveldb'};

      this.stores[name] =
          new (this.getRegisteredStore(type))({...options, name});
