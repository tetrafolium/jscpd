  ...
  public async detectByMap(tokenMap: TokensMap): Promise<IClone[]> {
    ...
    if (tokenMap.getLength() >= getOption('minTokens', this.options)) {
      const HashesStore: IStore<IMapFrame> =
          StoresManager.getStore(getHashDbName(tokenMap.getFormat())) as
          IStore<IMapFrame>;
      ...
      for (const mapFrame of tokenMap) {
        ...
        if (localDuplicate) {
          if (tokenMaps[tokenMaps.length - 1].localDuplicate) {
            initialMapFramesArray[initialMapFramesArray.length - 1].push(
                {...mapFrame, isClone : true});
          } else {
      ...
      const tokensStatuses: boolean[] =
          await HashesStore.hasKeys(tokenMaps.map(fr => fr.id));
      clones.push(...(await Promise.all(
          tokenMaps
              .reduce(
                  (mapFramesArray: IMapFrame[][], frame: IMapFrame,
                   index: number) => {
                    if (tokensStatuses[index]) {
                      if (!tokensStatuses[index - 1]) {
                        mapFramesArray.push([ {...frame, isClone : true} ]);
                      } else {
                        mapFramesArray[mapFramesArray.length - 1].push(
                            {...frame, isClone : true});
                      }
              ...
              .map((mapFrames: IMapFrame[]): Promise<IClone> => createClone(
                       mapFrames[0], mapFrames[mapFrames.length - 1])))));
      clones = clones.filter((clone: IClone): boolean => {
        const isAcceptableClone: boolean =
            isCloneLinesBiggerLimit(clone, getOption('minLines', this.options));
