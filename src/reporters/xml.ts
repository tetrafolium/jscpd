...
export class XmlReporter implements IReporter {
  ...
  public report(clones: IClone[]) {
    ...
    xmlDoc = this.options.xslHref
                 ? xmlDoc + '<?xml-stylesheet type="text/xsl" href="' +
    ...
    clones.forEach((clone: IClone) => {
      ...
      <duplication lines="${
          clone.duplicationA.end.line - clone.duplicationA.start.line}">
            <file path="${
          getPath(this.options, clone.duplicationA.sourceId)}" line="${
          clone.duplicationA.start.line}">
              <codefragment><![CDATA[${
          clone.duplicationA.fragment.replace(/]]>/i,
                                              'CDATA_END')}]]></codefragment>
            </file>
            <file path="${
          getPath(this.options, clone.duplicationB.sourceId)}" line="${
          clone.duplicationB.start.line}">
              <codefragment><![CDATA[${
          clone.duplicationB.fragment.replace(/]]>/i,
                                              'CDATA_END')}]]></codefragment>
            </file>
            <codefragment><![CDATA[${
          clone.duplicationA.fragment.replace(/]]>/i,
    ...
    writeFileSync(getOption('output', this.options) + '/jscpd-report.xml',
                  xmlDoc);
  }
}
