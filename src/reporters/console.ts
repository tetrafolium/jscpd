...
export class ConsoleReporter implements IReporter {
    ...
    if (statistic) {
      ...
      Object.keys(statistic.formats)
          ...
          .forEach((format: string) => {
            table.push(this.convertStatisticToArray(
                format, statistic.formats[format].total));
          });
  ...
  protected cloneFound(clone: IClone) {
    ...
    console.log('Clone found (' + format +
                '):' + (clone.isNew ? red('*') : ''));
    console.log(
        ` - ${getPathConsoleString(this.options, duplicationA.sourceId)} [${
            getSourceLocation(duplicationA.start, duplicationA.end)}]`);
    console.log(
        `   ${getPathConsoleString(this.options, duplicationB.sourceId)} [${
            getSourceLocation(duplicationB.start, duplicationB.end)}]`);
    console.log('');
  }

  private convertStatisticToArray(format: string,
                                  statistic: IStatisticRow): string[] {
    return [
      format, `${statistic.sources}`, `${statistic.lines}`,
      `${statistic.clones}`, `${statistic.duplicatedLines}`,
