...
const optionsDependencies = {
  fs : {existsSync : stub().returns(false)}
};

const {getDefaultOptions, prepareOptions} =
    proxyquire('../options', optionsDependencies);

const cli = new Command();

beforeEach(() => { cli.parse(process.argv); });
...
test('should make reporters array from string and add time reporter',
     (t: ExecutionContext) => {
       cli.reporters = '1,2';
...
test('should remove console reporters in silent mode',
     (t: ExecutionContext) => {
       cli.silent = true;
