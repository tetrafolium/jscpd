...
const useDependencies = {
  'jscpd-test-mode' : {default : {} as IMode},
  'jscpd-test-reporter' : {default : {} as IReporter},
  'detect-installed' : {sync : stub().returns(true)}
};
...
test('should throw error if module with mode does not exists',
     (t: ExecutionContext) => {
       useDependencies['detect-installed'].sync.returns(false);
       t.throws(() => { useMode('test1'); });
     });

test('should throw error if module with reporter does not exists',
     (t: ExecutionContext) => {
       useDependencies['detect-installed'].sync.returns(false);
       t.throws(() => { useReporter('test1'); });
     });
