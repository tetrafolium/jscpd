...
export function getOption(name: string, options?: IOptions): any {
  return options ? (options as any)[name] || (getDefaultOptions() as any)[name]
                 : (getDefaultOptions() as any)[name];
}

export function prepareOptions(cli: Command): IOptions {
  let config: string =
      cli.config ? resolve(cli.config) : resolve('.jscpd.json');
  ...
  if (existsSync(process.cwd() + '/package.json')) {
    packageJsonConfig =
        readJSONSync(process.cwd() + '/package.json').jscpd || {};
  }
  ...
  if (result.hasOwnProperty('config') && result.config &&
      isAbsolute(result.config) && result.path) {
    result.path =
        result.path.map((path: string) => resolve(dirname(config), path));
  }
  ...
  if (result.silent) {
    result.reporters =
        result.reporters.filter(reporter => reporter.indexOf('console') === -1)
            .concat('silent');
  }
