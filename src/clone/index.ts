...
export async function createClone(startMap: IMapFrame,
                                  endMap: IMapFrame): Promise<IClone> {
  const {format} = startMap;
  const hashesStore: IStore<IMapFrame> =
      StoresManager.getStore(getHashDbName(format)) as IStore<IMapFrame>;
  ...
  return {
    ...
    duplicationA : {
      ...
      fragment : getFragment(startMap.sourceId, startMap.start.range[0],
                             endMap.end.range[1])
    },
    duplicationB : {
      ...
      fragment : getFragment(sourceStart.sourceId, sourceStart.start.range[0],
                             sourceEnd.end.range[1])
    }
  };
}
...
export function getSourceFragment(source: ISourceOptions, start: number,
                                  end: number): string {
  return sourceToString(source).substring(start, end);
}

export function getSourceFragmentLength(source: ISourceOptions, start: number,
                                        end: number): number {
  return getSourceFragment(source, start, end).split('\n').length;
}

export function isCloneLinesBiggerLimit(clone: IClone,
                                        minLines: number): boolean {
  return clone.duplicationA.end.line - clone.duplicationA.start.line >=
         minLines;
}
