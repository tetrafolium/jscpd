...
const ignore = {
  ignore : [
    ...
    {pattern : /jscpd:ignore-start/, greedy : false},
    {pattern : /jscpd:ignore-end/, greedy : false}
  ]
...
(languages.markup as any).script.inside =
    {...ignore, ...(languages.markup as any).script.inside, ...punctuation};
(languages.markup as any).style.inside =
    {...ignore, ...(languages.markup as any).style.inside, ...punctuation};
...
export function initLanguages(langs: string[]): void {
  ...
  Object.keys(languages).forEach(lang => {
    languages[lang] = typeof languages[lang] === 'object'
...
export function tokenize(code: string, language: string): IToken[] {
  ...
  function createTokenFromString(token: string, lang: string): IToken[] {
    return [
      {format : lang, type : 'default', value : token, length : token.length} as
      IToken
    ];
  }
    ...
    if (token instanceof PrismToken && Array.isArray(token.content)) {
      ...
      token.content.forEach(
          t => (res = res.concat(createTokens(
                    t, token.alias ? sanitizeLangName(token.alias as string)
                                   : lang))));
      return res;
    }
  ...
  function calculateLocation(token: IToken): IToken {
    ...
    column = newLines !== 0 ? lines[lines.length - 1].length + 1
                            : column + lines[lines.length - 1].length;
    const end = {line : line + newLines, column};
  ...
  return tokens.map(calculateLocation).filter((t: IToken) => {
    return t.format !== 'important' && t.format !== 'property' &&
           t.format !== 'url';
  });
}
